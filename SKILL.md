...
- **If `osis.json` exists with `type: "org"`** → read the `products` and `modules` maps. Each entry is `"{slug}": "{path}"`; the slug is the routing key (kebab-case, matches the directory basename), the path is the location of that product's `osis/` tree. Ask what the user is working on today, listing products and modules with **both their display name and their path inline** so the resolution lands in working context up front. Render the slug as a clean display name: replace dashes with spaces and title-case, with judgment for acronyms (e.g. `sprint-os` → "Sprint OS", not "Sprint Os"). For products, route to the entry's path. For modules, route to the entry's path and read the module's `README.md` for its entry behavior before engaging. Products are full product trees with their own funnel docs; modules are activity wrappers that cut across products, with their own entry and phase playbooks.
